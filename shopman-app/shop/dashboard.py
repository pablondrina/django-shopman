...
def _build_pending_orders_table(today):
    ...
    for o in pending:
        ...
        rows.append([
            format_html('<a href="{}" class="font-medium">{}</a>', reverse("admin:ordering_order_change", args=[o.pk]), o.ref),
            format_html('<span class="inline-block rounded px-2 py-0.5 text-xs font-medium text-white {}">{}</span>', STATUS_BADGE_CSS.get(o.status, "bg-gray-500"), STATUS_LABELS.get(o.status, o.status)),
...
def _build_recent_orders_table(orders):
    ...
    for o in orders:
        ...
        rows.append([
            ...
            format_html('<span class="inline-block rounded px-2 py-0.5 text-xs font-medium text-white {}">{}</span>', css, o["status_label"]),
            o["total_display"],
            o["channel_name"],
...
def _build_production_table(wos):
    ...
    for wo in wos:
        rows.append([
            ...
            wo["output_ref"],
            str(wo["quantity"]),
            format_html('<span class="inline-block rounded px-2 py-0.5 text-xs font-medium text-white {}">{}</span>', WO_BADGE.get(wo["status"], "bg-gray-500"), WO_LABEL.get(wo["status"], wo["status"])),
        ])
...
def _build_operator_alerts_table(alerts):
    ...
    severity_icons = {"warning": "⚠️", "error": "❌", "critical": "🔴"}
    rows = []
    for alert in alerts:
