...
$(NUXT_DIR)/node_modules/.package-lock.json: $(NUXT_DIR)/package.json $(NUXT_DIR)/package-lock.json
	...
	cd $(NUXT_DIR) && npm install --silent
	@echo "✓ node_modules Nuxt pronto"

nuxt: $(NUXT_DIR)/node_modules/.package-lock.json ## Sobe Nuxt storefront (127.0.0.1:3000)
	@echo "── Nuxt storefront: http://127.0.0.1:3000 ──"
...
omotenashi-browser-ci: $(NUXT_DIR)/node_modules/.package-lock.json ## Gate local/CI: seed + Django + loja Nuxt + QA browser Omotenashi (port=8001 nuxt_port=3100)
	PYTHON="$(PYTHON)" \
...
seed: ## Popula banco com dados demo do deployment (flush + recria)
