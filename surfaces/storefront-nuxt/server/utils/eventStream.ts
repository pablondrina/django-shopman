...
// fazemos streaming do eventstream do Django, sem materializar o corpo.
//
// Same-origin de propósito: não há CORS no projeto e o cookie `.boulangerie`
// viaja no proxy. Diferente do proxyDjangoApi (que bufferiza), SSE é corpo
// infinito — precisa de `fetch` nativo devolvendo o ReadableStream. Se o Django
// recusar (404 p/ não-autorizado), propagamos o status para o EventSource falhar
// de vez (não reconecta) e a página seguir no fallback (poll).
//
// Ver ADR SSE-first (docs/decisions) — a preferência é push por SSE em cima de um
// fetch canônico que continua sendo a fonte da verdade.
