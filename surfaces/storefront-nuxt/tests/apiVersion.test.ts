// Checagem do X-API-Version no BFF do storefront: parse da major e warning
// estruturado em divergência — sem nunca quebrar o request (graceful). O
// carimbo vem do APIVersionHeaderMiddleware (shopman/shop/middleware.py).
...
describe('warnOnApiVersionMismatch', () => {
  ...
  it('stays silent when the major matches', () => {
    ...
    warnOnApiVersionMismatch(String(EXPECTED_API_MAJOR), { path: '/api/v1/storefront/cart/' })
  ...
  it('logs a structured warning when the major diverges', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
    warnOnApiVersionMismatch('2', { path: '/api/v1/storefront/cart/' })
