  ...
  <main class="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 p-6 md:p-10">
    ...
    <ul v-else class="grid gap-3">
      <li v-for="inst in instances" :key="inst.ref">
        ...
        >
          <span
            class="grid size-12 shrink-0 place-items-center rounded-md bg-background text-muted-foreground"
          >
          ...
          <span class="min-w-0 flex-1">
            <span
              ...
              >{{ inst.type_display }}</span
            >
            <span class="block truncate text-xl font-bold leading-tight">{{
              inst.name
            }}</span>
          </span>
          <span
