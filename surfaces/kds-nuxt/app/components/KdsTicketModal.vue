...
import { elapsedLabel, itemProgress, lucideIcon, slaPercent, splitRef, targetLabel, ticketTone, toneBar, toneTimer } from "~/presentation/board";

const props = defineProps<{ open: boolean; ticket: KDSTicketProjection | null }>();
...
const fill = computed(() => (props.ticket ? slaPercent(props.ticket.elapsed_seconds, props.ticket.target_seconds) : 0));
const progress = computed(() => (props.ticket ? itemProgress(props.ticket.items) : { done: 0, total: 0 }));
</script>
...
<template>
  <UiDialog :open="open" @update:open="$emit('update:open', Boolean($event))">
    <UiDialogContent v-if="ticket" class="flex max-h-[90vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-lg">
      <UiDialogTitle class="sr-only">Pedido {{ ticket.order_ref }}</UiDialogTitle>
      <UiDialogDescription class="sr-only">
        {{ ticket.customer_name || "Sem cliente" }} — {{ progress.done }} de {{ progress.total }} itens prontos.
      </UiDialogDescription>
      <!-- header -->
      <div class="border-b">
        <div class="flex items-start justify-between gap-3 p-5">
          <div class="min-w-0">
            <p class="flex items-center gap-1.5 text-[0.7rem] font-medium uppercase tracking-wide text-muted-foreground">
              <Icon v-if="ticket.channel_icon" :name="`lucide:${lucideIcon(ticket.channel_icon)}`" class="size-3.5" />
              <Icon v-if="ticket.fulfillment_icon" :name="`lucide:${lucideIcon(ticket.fulfillment_icon)}`" class="size-3.5" />
              <span class="tabular-nums">{{ ref_.prefix }}{{ ticket.created_at_display }}</span>
            </p>
            <p class="break-words text-4xl font-extrabold tracking-tight tabular-nums leading-none">{{ ref_.code }}</p>
            <p v-if="ticket.customer_name" class="mt-1.5 truncate text-sm font-medium text-foreground/80">{{ ticket.customer_name }}</p>
          </div>
          <div class="flex shrink-0 flex-col items-end gap-0.5 rounded-md border px-3 py-2 text-right" :class="timerClasses">
            <span class="flex items-center gap-1 text-2xl font-bold tabular-nums leading-none">
              <Icon name="lucide:timer" class="size-5 opacity-70" />
              {{ elapsedLabel(ticket.elapsed_seconds) }}
            </span>
            <span v-if="ticket.target_seconds" class="text-[0.65rem] font-medium uppercase tracking-wide opacity-60">
      ...
      <div class="min-h-0 flex-1 overflow-y-auto p-3">
        <div class="flex items-center justify-between px-1 pb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
          <span>Itens</span>
          ...
          <li v-for="(item, idx) in ticket.items" :key="idx">
            ...
            >
              <span class="min-w-0 flex-1">
                <span class="relative flex items-center gap-2 text-lg leading-snug transition-colors duration-200" :class="item.checked ? 'text-muted-foreground' : 'font-bold'">
                ...
                <span v-if="item.notes" class="mt-1.5 flex" :class="item.checked ? 'opacity-50' : ''">
                  <span class="inline-flex min-w-0 max-w-full items-center rounded border bg-muted px-2 py-0.5 text-sm font-medium text-muted-foreground">
                    <span class="truncate">{{ item.notes }}</span>
                  </span>
                </span>
                <span v-if="item.stock_warning" class="mt-1.5 flex" :class="item.checked ? 'opacity-50' : ''">
                  <span class="inline-flex min-w-0 max-w-full items-center gap-1.5 rounded border bg-muted px-2 py-0.5 text-sm font-semibold text-foreground">
      ...
      <div class="shrink-0 border-t p-4">
        <button
          ...
          :class="ticket.all_checked
            ? 'border-border bg-background text-foreground hover:bg-accent'
            : 'border-border/60 text-muted-foreground hover:bg-accent hover:text-foreground'"
          @click="$emit('done')"
        >
          <Icon name="lucide:check-check" class="size-5" :class="ticket.all_checked ? 'text-green-400' : ''" />
