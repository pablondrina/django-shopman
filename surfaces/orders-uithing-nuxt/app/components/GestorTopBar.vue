...
const section = computed(() => (route.path.startsWith("/catalog") ? "catalog" : "orders"));
...
<template>
  <header class="flex shrink-0 items-center gap-3 border-b bg-card px-4 py-2 print:hidden">
    ...
    <nav class="ml-1 inline-flex items-center rounded-md border bg-muted/40 p-0.5">
      <NuxtLink
        v-for="t in tabs"
        :key="t.key"
        :to="t.to"
        class="inline-flex h-8 items-center gap-1.5 rounded px-3 text-sm transition"
        :class="section === t.key ? 'bg-card font-medium text-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'"
      >
