...
describe('surface UX guardrails', () => {
  ...
  it('does not render a second command search field in the menu', () => {
    ...
    expect(menu).toContain('searchResultOptions')
    ...
    expect(menu).toContain('[margin-left:calc(50%_-_50vw)]')
    ...
    expect(menu).toContain('<UiItemGroup')
  ...
  it('keeps checkout authentication driven by the projection contract', () => {
    ...
    expect(checkout).toContain("type Step = 'fulfillment' | 'address' | 'when' | 'payment'")
    ...
    expect(checkout).toContain('<UiRadioGroup v-model="state.fulfillment_type" class="sm:grid-cols-2">')
    expect(checkout).toContain('<UiRadioGroup v-model="state.payment_method" class="sm:grid-cols-2">')
    ...
    expect(checkout).toContain('<UiFieldTitle>{{ method.label }}</UiFieldTitle>')
  ...
  it('uses canonical guide-line subcomponents for tracking timeline and keeps checkout as progressive sections', () => {
    ...
    expect(checkout).toContain('isDone')
    expect(checkout).toContain('isUpcoming')
    expect(checkout).not.toContain('<UiStepper')
