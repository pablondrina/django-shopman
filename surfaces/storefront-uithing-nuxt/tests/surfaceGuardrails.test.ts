...
describe('surface UX guardrails', () => {
  ...
  it('does not render a second command search field in the menu', () => {
    ...
    expect(menu).toContain('allFilteredCount')
    ...
    expect(menu).toContain('resultSections')
    expect(menu).toContain('activeSectionCounts')
    expect(menu).toContain('collectionSearchOptions')
    expect(menu).toContain('productSearchOptions')
    expect(menu).toContain('keywordSearchOptions')
    expect(menu).toContain('collectionSearchScore')
    expect(menu).toContain('productSearchScore')
    expect(menu).toContain('matchesProductAcrossCatalog')
    expect(menu).toContain('searchResultGroups')
    ...
    expect(menu).toContain('chooseSectionFromSearch')
    expect(menu).toContain('chooseSearchResult')
    expect(menu).toContain('<UiListbox')
    expect(menu).toContain('<UiListboxGroupLabel')
    expect(menu).toContain('<UiListboxFilter v-model="query" as-child auto-focus>')
    expect(menu).toContain('data-menu-search-listbox')
    expect(menu).toContain('data-menu-filter-result-list')
    expect(menu).toContain('data-menu-collection-list')
    expect(menu).toContain('hasPendingFilterSelection')
    ...
    expect(menu).toContain('applySelectedMenuFilters')
    expect(menu).toContain('toggleFilterKey')
    expect(menu).toContain('itemMatchesAnyFilter')
    expect(menu).toContain("kind: 'collection'")
    expect(menu).toContain("kind: 'product'")
    expect(menu).toContain("kind: 'keyword'")
    expect(menu).toContain('lucide:funnel')
    expect(menu).toContain('if (!searchOrFilterMode.value) return true')
    expect(menu).toContain('return resultSections.value')
  ...
  it('uses canonical Card defaults intentionally for product media cards', () => {
    ...
    expect(productTile).toContain('<UiPopoverTrigger as-child>')
    expect(productTile).toContain('<UiPopover v-if="item.available_qty != null">')
    expect(productTile).toContain('<UiPopoverContent class="w-64 border-foreground/15 bg-foreground text-background">')
    expect(productTile).toContain("formatCount(item.available_qty, 'unidade disponível', 'unidades disponíveis') }} no momento")
  ...
  it('keeps mobile product add controls explicit before switching to quantity editing', () => {
    ...
    expect(quantity).toContain("props.tone === 'inverted' ? 'border-background/30 bg-background text-foreground' : ''")
