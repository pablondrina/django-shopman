...
class TestValidateSlot(TestCase):
    def _call(self, slot_ref: str, fulfillment: str, delivery_date: str = "") -> dict:
        with patch("shopman.services.pickup_slots.get_slots", return_value=FAKE_SLOTS):
    ...
    def _call_at_hour(
        ...
        fake = _fake_now(hour)
        with (
            patch("shopman.services.pickup_slots.get_slots", return_value=FAKE_SLOTS),
            patch("shopman.web.views.checkout.timezone.localtime", return_value=fake),
        ):
            return CheckoutView._validate_slot(slot_ref, fulfillment, delivery_date)
