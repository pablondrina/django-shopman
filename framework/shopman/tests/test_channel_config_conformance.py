...
def _patch_config(cfg: ChannelConfig):
    ...
    return patch("shopman.lifecycle.ChannelConfig.for_channel", return_value=cfg)
...
class TestConfirmationConformance:
    def test_immediate_auto_confirms_on_commit(self):
        ...
        patches = [
            patch("shopman.lifecycle.customer.ensure"),
            patch("shopman.lifecycle.stock.hold"),
            patch("shopman.lifecycle.loyalty.redeem"),
            patch("shopman.lifecycle.payment.initiate"),
            patch("shopman.lifecycle.fulfillment.create"),
            _patch_config(cfg),
        ]
...
class TestPaymentConformance:
    def test_external_timing_skips_initiate(self):
        ...
        with _patch_config(cfg):
            with patch("shopman.lifecycle.customer.ensure"):
                with patch("shopman.lifecycle.stock.hold"):
                    with patch("shopman.lifecycle.loyalty.redeem"):
                        with patch("shopman.lifecycle.payment.initiate") as mock_init:
                            dispatch(order, "on_commit")
    ...
    def test_post_commit_initiates_on_confirmed(self):
        ...
        with _patch_config(cfg):
            with patch("shopman.lifecycle.payment.initiate") as mock_init:
                with patch("shopman.lifecycle.stock.fulfill"):
                    with patch("shopman.lifecycle.notification.send"):
    ...
    def test_at_commit_creates_fulfillment_on_commit(self):
        ...
        with _patch_config(cfg):
            with patch("shopman.lifecycle.customer.ensure"):
                with patch("shopman.lifecycle.stock.hold"):
                    with patch("shopman.lifecycle.loyalty.redeem"):
                        with patch("shopman.lifecycle.fulfillment.create") as mock_create:
                            dispatch(order, "on_commit")
...
class TestCascadeConformance:
    def test_channel_overrides_defaults(self):
        ...
        from shopman.config import deep_merge
