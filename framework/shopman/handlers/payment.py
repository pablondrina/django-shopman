...
class PixTimeoutHandler:
    ...
    def handle(self, *, message: Directive, ctx: dict) -> None:
        ...
        if cancel(order, reason="pix_timeout", actor="pix.timeout"):
            Directive.objects.create(
                topic=NOTIFICATION_SEND,
...
class PaymentTimeoutHandler:
    ...
    def handle(self, *, message: Directive, ctx: dict) -> None:
        ...
        reason = f"{method}_timeout"
        if cancel(order, reason=reason, actor=f"{method}.timeout"):
            Directive.objects.create(
                topic=NOTIFICATION_SEND,
                payload={
                    "order_ref": order_ref,
