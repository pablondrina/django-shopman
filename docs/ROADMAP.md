...
| PostgreSQL pequeno exige disciplina de conexões | `DATABASE_CONN_MAX_AGE=60` reduziu latência, mas voltou a saturar o Postgres pequeno com conexões ociosas e gerou 500/503 intermitente. | Manter `CONN_MAX_AGE=0` no staging até existir pool de conexões ou plano maior; depois testar novo valor com métrica de conexões. |
