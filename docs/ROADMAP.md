...
| PostgreSQL pequeno exige disciplina de conexões | Incidente 500 por `too many clients already` foi contido; para reduzir latência de requests curtos o blueprint voltou a `DATABASE_CONN_MAX_AGE=60`. | Monitorar conexões; se saturar, migrar para pool/instância maior em vez de manter `CONN_MAX_AGE=0`. |
