...
| Shelf life perecível — precedência agora explícita | `Product.shelf_life_days` (janela relativa) e `Batch.expiry_date` (data absoluta do lote) são aplicados em **AND** (mais restritivo vence) em `quants_eligible_for`; contrato travado por teste (`TestShelflifeBatchPrecedence`). `shelf_life_days` já é canônico e exposto via SkuValidator. | Resta (decisão de arquitetura): onde validar consistência `expiry_date ≤ production_date + shelf_life_days` no registro de lote, sem acoplar Stockman↔Offerman (provável: orquestrador/admin form). |
