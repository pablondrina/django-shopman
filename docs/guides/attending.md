# Attending — Clientes e Relacionamento

## Visão Geral

O app `shopman.attending` gerencia clientes, contatos, endereços, segmentação, loyalty, insights (RFM/LTV) e consentimento (LGPD). É o CRM do ecossistema shopman.
...
**Propriedades:** `name` (nome completo), `listing_code` (código de preço do grupo), `default_address`
...
| `listing_code` | CharField(50) | Link para Listing (Offering) |
...
```python
from shopman.attending.services.customer import customer
...
# CustomerValidation(valid=True, listing_code="atacado", ...)

# Buscar preço do grupo
listing_code = customer.get_listing_code("CLI-001")
...
```python
from shopman.attending.services.address import addresses, add_address, set_default_address
...
```python
from shopman.attending.contrib.identifiers.service import IdentifierService
...
- `CustomerInfo(ref, name, customer_type, group_ref, listing_code, phone, email, default_address, total_orders, is_vip, is_at_risk, favorite_products)`
...
```python
from shopman.attending.contrib.manychat.service import ManychatService
...
```python
from shopman.attending.contrib.merge.service import MergeService
