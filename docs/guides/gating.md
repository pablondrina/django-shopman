# Gating — Autenticação Passwordless

## Visão Geral

O app `shopman.gating` implementa autenticação passwordless com três fluxos de login (OTP, Bridge Token, Magic Link), confiança de dispositivo e rate limiting. Não depende de senhas — a identidade é verificada via código enviado por WhatsApp/SMS/Email.
...
### Identity Link
Mapeamento Django User ↔ Customer (Attending). Desacopla autenticação de gestão de clientes.
...
| `customer_id` | UUIDField(unique) | UUID do cliente no Attending |
...
# VerifyResult(success=True, customer=<GatingCustomerInfo>, created_customer=False)
```
...
# AuthResult(success=True, user=<User>, customer=<GatingCustomerInfo>)
```
...
```python
from shopman.gating.services.device_trust import DeviceTrustService
...
Interface para resolver clientes (desacopla do Attending).
...
| Desacoplamento | CustomerResolver desacopla do Attending |
