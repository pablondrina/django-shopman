...
O módulo `shopman/` (em `shopman-app/shopman/`) é o orquestrador do sistema. Ele não contém lógica de domínio — apenas **coordena** os apps core (offering, stocking, crafting, ordering, customers, auth, payments) através de 4 conceitos separados:
