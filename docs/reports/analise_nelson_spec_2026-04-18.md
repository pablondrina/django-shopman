...
- [instances/nelson/apps.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/apps.py:1)
- [instances/nelson/modifiers.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/modifiers.py:1)
- [instances/nelson/customer_strategies.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/customer_strategies.py:1)
- [instances/nelson/management/commands/seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1)
- [instances/nelson/static/icon-192.svg](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/static/icon-192.svg:1)
- [instances/nelson/static/icon-512.svg](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/static/icon-512.svg:1)

## O que a instancia realmente e

- `NelsonConfig` so define identidade de app: nome, label e `verbose_name`, sem `ready()` proprio, sem bootstrap local e sem declaracao de comportamento adicional ([apps.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/apps.py:1)).
...
- A loja e uma boulangerie artesanal em Londrina, com marca, razao social, endereco completo, DDD padrao, horario de funcionamento e paleta cromatica explicitamente definidos no seed ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:119)).
...
- O seed preenche `social_links`, `opening_hours`, `pickup_slots`, `pickup_slot_config`, `closed_dates`, `seasons`, `high_demand_multiplier` e `safety_stock_percent` ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:166)).
- O `Shop` do core suporta exatamente esse tipo de especializacao via `defaults`, `branding`, `opening_hours` e `social_links` ([shop.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/models/shop.py:118)).

### Canais e comportamento operacional

- A instância define cinco canais: `balcao`, `delivery`, `ifood`, `whatsapp` e `web` ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1315)).
- `balcao` e POS puro: confirmacao imediata, pagamento em dinheiro/external, checagem de estoque no commit e UX de operador com `handle_label="Comanda"` e placeholder curto ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1319)).
- `delivery` e `web` compartilham um perfil remoto: auto-confirmacao com timeout, PIX/cartao, `hold_ttl_minutes=30` e exclusao de `ontem` do escopo de estoque ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1330)).
- `ifood` e o canal marketplace: confirmacao manual, pagamento externo, `pricing.policy="external"`, `editing.policy="locked"` e alerta de pedido novo estagnado ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1339)).
- `whatsapp` recebe backend `manychat`, mantendo o canal conversa-first como frente propria do atendimento ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1344)).
- O contrato do core para isso e claro: `Channel.kind` descreve comportamento, `Channel.config` carrega overrides e `ChannelConfig.for_channel()` resolve cascata `defaults -> shop.defaults -> channel.config` ([channel.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/models/channel.py:14), [config.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/config.py:191)).

### Estoque, D-1 e disponibilidade

- A instancia modela quatro posicoes: `deposito`, `vitrine`, `producao` e `ontem` ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:751)).
...
- O seed materializa estoque inicial na vitrine e um subconjunto D-1 em `ontem`, depois cria alertas de minimo por SKU ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:777), [seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1678)).
- O core de `ChannelConfig.Stock` suporta exatamente essa semantica com `allowed_positions`, `excluded_positions` e `hold_ttl_minutes` ([config.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/config.py:79)).

### Catalogo e sortimento

- O seed cria um catalogo extensivo, com paes, focaccias, brioches, folhados, doces, salgados, lanches, cafes e combos, e associa keywords por SKU para busca e alternativas ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:327)).
...
- A instancia nao so cadastra receitas como tambem exercita o motor de producao com `Recipe`, `RecipeItem`, `WorkOrder` e `WorkOrderItem` ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:850)).
...
- Ha tres grupos de cliente: `varejo`, `atacado` e `staff` ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1258)).
...
- O seed gera 35 dias de pedidos historicos mais alguns pedidos "live" para simular a operacao de cozinha e atendimento ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:1382)).
...
- A instancia usa `LoyaltyService` para inscrever clientes, acumular pontos, acumular stamps e resgatar pontos, simulando historico real de fidelidade ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:2046)).
- O KDS e dividido em `lanches`, `cafes`, `encomendas` e `expedicao`, com tipos `prep`, `picking` e `expedition` e tempos alvo distintos ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:2116), [kds.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/models/kds.py:8)).
- `DayClosing` e `CashRegisterSession` nao sao decorativos: a instancia realmente preenche fechamento, sangria, abertura e diferenca de caixa ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:2292), [cash_register.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/models/cash_register.py:12)).
...
- `D1DiscountModifier` aplica desconto sobre itens marcados como D-1 via `session.data["availability"]` ou `item["is_d1"]`, com percentual configuravel por canal ([modifiers.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/modifiers.py:34)).
...
- `HappyHourModifier` usa janela horaria, exclui o canal web e ignora itens com desconto de funcionario, o que reduz conflito entre promocao e politica interna ([modifiers.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/modifiers.py:94)).
- A integracao com o core e boa: `SHOPMAN_INSTANCE_MODIFIERS` carrega classes por dotted path, e o registry do orchestrador instancia cada modifier no boot ([handlers/__init__.py](/Users/pablovalentini/Dev/Claude/django-shopman/shopman/shop/handlers/__init__.py:181), [settings.py](/Users/pablovalentini/Dev/Claude/django-shopman/config/settings.py:646)).

### `balcao` como estrategia de cliente

- A estrategia `nelson_handle_balcao()` resolve cliente em ordem: telefone, CPF, walk-in anonimo ([customer_strategies.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/customer_strategies.py:17)).
...
- O seed se apresenta como "producao", mas usa muitos placeholders: `contato@example.com`, links sociais de exemplo e imagens vindas de um repositório externo no GitHub ([seed.py](/Users/pablovalentini/Dev/Claude/django-shopman/instances/nelson/management/commands/seed.py:330)).
