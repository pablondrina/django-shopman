...
| `lifecycle` | `dict` | `lifecycle._mark_phase_complete()` | `sweep_stuck_orders` | Marcador durável de conclusão de fase: `{on_commit: "done"}`. on_commit não é durável (roda no `on_commit` do signal); um crash entre o COMMIT e o callback deixaria o pedido órfão em NEW. O sweeper re-despacha, idempotente, os NEW antigos sem `on_commit=done` |
