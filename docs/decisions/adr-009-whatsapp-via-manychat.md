...
- Envio de OTP via template de WhatsApp (ManyChat flow).
...
2. **Fallback chain em notificações**: `manychat → sms → email` já é padrão, reduz dependência single-point (mas não elimina para OTP se primeiro for WhatsApp).
