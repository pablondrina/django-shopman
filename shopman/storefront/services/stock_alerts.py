...
def has_pending(sku: str) -> bool:
    """Cheap guard for the stock-arrival receiver (indexed exists())."""
...
def subscribe(sku: str, *, channel_ref: str = "web", customer=None, phone: str = ""):
    ...
    Dedupes a pending alert per (sku, target). ``customer`` is a Guestman
    Customer (or None for anonymous); ``phone`` is the anonymous contact.
    """
    from shopman.storefront.models import StockAlertSubscription
    ...
    pending = StockAlertSubscription.objects.filter(sku=sku, notified_at__isnull=True)
    existing = (
...
def notify_back_in_stock(sku: str) -> int:
    """Notify pending subscribers of ``sku`` once it is available again.
    ...
    pending = list(StockAlertSubscription.objects.filter(sku=sku, notified_at__isnull=True))
    if not pending:
        return 0
    ...
    for sub in pending:
        ...
        if _deliver(sub, product_name=product_name):
            sub.notified_at = timezone.now()
            sub.save(update_fields=["notified_at"])
            notified += 1
    if notified:
        logger.info("stock_alerts: notified %s subscriber(s) for sku=%s", notified, sku)
    return notified
...
def _deliver(sub, *, product_name: str) -> bool:
    """Send the back-in-stock notification via the channel's backend. True on success."""
