"""Generate the KDS contract mirror consumed by the kds-nuxt surface.

The contract's single source of truth is the projection dataclasses in
``shopman.backstage.projections.kds``. The KDS app used to hand-sync those
shapes in TypeScript — a fragile manual mirror. This command renders them into
a generated TypeScript module, so the surface imports (and narrows) them
instead of re-declaring them.

A drift test (``test_kds_schema_export``) regenerates the file in-memory and
compares it to disk, failing loudly when the two diverge. Run::

    python manage.py export_kds_schema
...
OUTPUT_RELATIVE_PATH = Path("surfaces/kds-nuxt/app/generated/kdsContract.ts")
...
def render_kds_contract_ts() -> str:
    """Render the generated TypeScript contract mirror (deterministic)."""
    return render_contract_module(
        source="shopman/backstage/projections/kds.py",
        command="export_kds_schema",
        dataclasses=CONTRACT_DATACLASSES,
    )


class Command(BaseCommand):
    help = "Generate the KDS contract mirror (TypeScript) from the projections."
