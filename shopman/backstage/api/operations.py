...
POST endpoints (operator actions):
  ...
  POST /api/v1/backstage/pos/cash/open/              → open cash session
  POST /api/v1/backstage/pos/cash/close/             → close cash session
...
@extend_schema_view(
    post=extend_schema(
        ...
        responses={200: OpenApiResponse(description="Cash session opened.")},
    ),
)
class POSCashOpenView(APIView):
    ...
    def post(self, request):
        ...
        try:
            session = pos_service.open_cash_session(
...
@extend_schema_view(
    post=extend_schema(
        ...
        responses={200: OpenApiResponse(description="Cash session closed.")},
    ),
)
class POSCashCloseView(APIView):
    ...
    def post(self, request):
        ...
        try:
            result = pos_service.close_cash_session(
                operator=request.user,
        ...
        return Response({"ok": True, "result": projection_data(result) if result else None})
