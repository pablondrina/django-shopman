...
_PATCHES = [
    ...
    "shopman.shop.services.stock.hold",
    "shopman.shop.services.stock.fulfill",
    "shopman.shop.services.stock.release",
    "shopman.shop.services.stock.revert",
    ...
    "shopman.shop.services.fiscal.emit",
    "shopman.shop.services.fiscal.cancel",
    "shopman.shop.services.kds.dispatch",
    "shopman.shop.services.kds.cancel_tickets",
...
class TestC11StockUnavailableAtCommit(TestCase):
    ...
    def test_order_cancelled_when_sku_unavailable(self):
        ...
        patchers, mocks = _start_patches(overrides={
            "shopman.shop.services.availability.decide": avail_fail,
        })
        try:
            session = _session(self.channel)
            result = _commit(session, self.channel)
...
class TestC12PartialQtyHold(TestCase):
    ...
    def test_partial_hold_triggers_cancel(self):
        ...
        _partial_patches = [t for t in _PATCHES if t != "shopman.shop.services.stock.hold"]
        patchers = []
        ...
        try:
            with patch("shopman.shop.services.stock.get_adapter", return_value=mock_adapter), \
...
class TestC13BundleHoldExpansion(TestCase):
    ...
    def test_bundle_hold_expansion(self):
        ...
        try:
            with patch("shopman.shop.services.stock.get_adapter", return_value=mock_adapter), \
                 patch("shopman.shop.services.stock._expand_if_bundle", side_effect=_expand):
                session = _session(self.channel, items=[
