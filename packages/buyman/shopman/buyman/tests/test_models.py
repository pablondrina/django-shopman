...
class TestMaterial:
    def test_basics_and_perishable(self):
        m = Material.objects.create(sku="INS-FARINHA-T65", name="Farinha T65", unit="kg", shelf_life_days=180)
        assert str(m).startswith("INS-FARINHA-T65")
        assert m.is_perishable is True

    def test_non_perishable_when_no_shelf_life(self):
        m = Material.objects.create(sku="INS-SAL", name="Sal", unit="kg")
        assert m.is_perishable is False

    def test_sku_unique(self):
        Material.objects.create(sku="INS-X", name="X")
        with pytest.raises(IntegrityError), transaction.atomic():
            Material.objects.create(sku="INS-X", name="Y")


class TestSupplierMaterialCost:
    def test_cost_per_pair_unique(self):
        s = Supplier.objects.create(ref="SUP-MOINHO", name="Moinho SP")
        m = Material.objects.create(sku="INS-FARINHA-T65", name="Farinha")
    ...
    def test_one_preferred_cost_per_material(self):
        ...
        m = Material.objects.create(sku="INS-FARINHA", name="Farinha")
