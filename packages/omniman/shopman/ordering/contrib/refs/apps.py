...
class RefsConfig(AppConfig):
    name = "shopman.ordering.contrib.refs"
    label = "refs"
    verbose_name = _("Referências")

    def ready(self):
        """Registra RefTypes padrão quando a app carrega."""
        from shopman.ordering.contrib.refs.registry import register_ref_type
        from shopman.ordering.contrib.refs.types import DEFAULT_REF_TYPES
