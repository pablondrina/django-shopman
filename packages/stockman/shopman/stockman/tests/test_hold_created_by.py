"""
Tests for Hold.created_by field and actor params on lifecycle methods.
"""
...
class TestHoldCreatedBy:
    def test_hold_stores_created_by(self, prod, pos, today, op):
        stock.receive(Decimal('10'), prod.sku, pos, reason='Entrada')
        hold_id = stock.hold(Decimal('3'), prod, today, created_by=op)

        hold = Hold.objects.get(pk=int(hold_id.split(':')[1]))
        assert hold.created_by == op

    def test_hold_created_by_none_by_default(self, prod, pos, today):
        stock.receive(Decimal('10'), prod.sku, pos, reason='Entrada')
        hold_id = stock.hold(Decimal('3'), prod, today)

        hold = Hold.objects.get(pk=int(hold_id.split(':')[1]))
        assert hold.created_by is None

    def test_hold_demand_stores_created_by(self, prod, today, op):
        """demand_ok policy: created_by also stored on demand hold."""
        from types import SimpleNamespace
        demand_prod = SimpleNamespace(
            sku='CB-DEMAND', name='Demand CB',
            shelf_life_days=None, availability_policy='demand_ok',
        )
        hold_id = stock.hold(Decimal('5'), demand_prod, today, created_by=op)
