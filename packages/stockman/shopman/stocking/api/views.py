...
from shopman.stocking.models import Hold, Move, Position, Quant
...
def _availability_for_skus(
    ...
    *,
    ...
    """
    Batch version of _availability_for_sku() — same logic, 4 queries regardless of N.

    Returns {sku: availability_dict} for all requested SKUs. Functionally identical
    to calling _availability_for_sku(sku, safety_margin=..., allowed_positions=...)
    for each SKU, but uses bulk DB queries to avoid N+1.
    """
    from shopman.offering.models import Product
    from shopman.stocking.models import Batch, Hold
    ...
    for sku in skus:
        ...
        for quant in quants_by_sku.get(sku, []):
            ...
            # Classify into breakdown buckets (same logic as _availability_for_sku)
            if quant.batch == "D-1":
                d1 += qty
                held_d1 += held
            elif quant.position and quant.position.kind == "process":
