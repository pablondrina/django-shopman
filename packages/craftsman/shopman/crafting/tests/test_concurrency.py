...
These tests verify that craft.close() with select_for_update() and the
...
How close() protects against concurrent modifications:
  1. Acquires row lock via select_for_update()
  2. Refreshes the caller's object in-place to get current DB state
  3. Checks status == OPEN (raises CraftError('TERMINAL_STATUS') if not)
...
Two concurrent close() calls on the same WO:
  - First one acquires the lock, transitions to DONE, commits
  - Second one acquires the lock after commit, reads status=DONE, raises
...
from shopman.crafting import craft, CraftError, StaleRevision
...
@requires_postgres
class TestConcurrentCloseWorkOrder(TransactionTestCase):
    """
    Two threads both try to close the same WorkOrder.
    Only one should succeed; the other should get CraftError('TERMINAL_STATUS')
    because after the first close commits, the WO status is 'done', which
    select_for_update + refresh_from_db reveals to the second thread.
    """

    def test_concurrent_close_work_order(self):
        recipe = _make_recipe("close-conc-001")
        wo = craft.plan(recipe, Decimal("50"))

        # Both threads will try to close the same WO simultaneously
        results = []
        barrier = threading.Barrier(2)

        def attempt_close(thread_id, produced_qty):
            try:
                barrier.wait()
                # Each thread uses the same wo reference but close() locks
                # and refreshes the row atomically inside the transaction
                craft.close(wo, produced=produced_qty)
        ...
        t1 = threading.Thread(target=attempt_close, args=(1, Decimal("48")))
        t2 = threading.Thread(target=attempt_close, args=(2, Decimal("45")))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
        ...
        # WO must be DONE with a single produced value
        wo.refresh_from_db()
        self.assertEqual(wo.status, WorkOrder.Status.DONE)
        self.assertIsNotNone(wo.produced)


@requires_postgres
class TestCloseWithStaleRevRaisesStaleRevision(TransactionTestCase):
    """
    ...
    def test_close_with_stale_rev_raises(self):
        ...
        with self.assertRaises(StaleRevision) as ctx:
            craft.close(wo, produced=Decimal("90"), expected_rev=current_rev - 1)

        self.assertEqual(ctx.exception.code, "STALE_REVISION")

        # WO must still be OPEN — the stale close did not take effect
        wo.refresh_from_db()
        self.assertEqual(wo.status, WorkOrder.Status.OPEN)
        self.assertIsNone(wo.produced)
