...
from shopman.auth.conf import get_adapter, reset_adapter
...
@override_settings(AUTH={"AUTH_ADAPTER": "shopman.auth.tests.test_adapter.CustomAdapter"})
...
def test_create_and_send_email(customer):
    ...
    with patch("shopman.auth.adapter.DefaultAuthAdapter.send_access_link", return_value=True):
...
def test_create_and_send_failure(customer):
    ...
    with patch("shopman.auth.adapter.DefaultAuthAdapter.send_access_link", return_value=False):
        with patch.object(AccessLinkService, "_build_url", return_value="https://test.local/link"):
