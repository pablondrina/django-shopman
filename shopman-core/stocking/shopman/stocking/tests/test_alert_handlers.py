...
class TestCooldown:
    ...
    def test_cooldown_prevents_re_dispatch(self, stocked_product, vitrine, alert, settings):
        ...
        with patch("shopman.stocking.contrib.alerts.handlers._dispatch_notification") as mock_dispatch:
            stock.issue(Decimal("35"), quant, reason="Sale")
...
class TestDirectiveCreation:
    ...
    def test_dispatch_graceful_without_omniman(self, stocked_product, vitrine, alert):
        ...
        with patch(
            "shopman.stocking.contrib.alerts.handlers._dispatch_notification"
        ) as mock_dispatch:
