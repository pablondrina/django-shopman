...
from shopman.ordering.admin_widgets import DatalistTextInput
from shopman.ordering.exceptions import CommitError, IssueResolveError, SessionError
...
from shopman.ordering.services import CommitService, ResolveService
...
class SessionAdmin(ModelAdmin):
    ...
    def action_commit(self, request: HttpRequest, obj: Session):
        ...
        if obj.state != "open":
            ...
            return HttpResponseRedirect(reverse("admin:ordering_session_change", args=[obj.pk]))
        ...
        try:
            ...
            if executed_count > 0:
                messages.success(
                    ...
                    format_html(
                        ...
                        order_ref,
                        executed_count,
                        reverse("admin:ordering_order_change", args=[result.get("order_id")]),
                    ),
                )
            else:
                messages.success(
            ...
            if order_id:
                return HttpResponseRedirect(reverse("admin:ordering_order_change", args=[order_id]))
            else:
                # Fallback: redireciona para changelist com filtros
                return HttpResponseRedirect(
                    reverse("admin:ordering_order_changelist") + f"?status__exact=new&ref={order_ref}"
                )
    ...
    def resolve_issue_view(self, request, object_id, issue_id, action_id):
        ...
        if session is None:
            ...
            return HttpResponseRedirect(reverse("admin:ordering_session_changelist"))
        try:
        ...
        return HttpResponseRedirect(
            reverse("admin:ordering_session_change", args=[object_id])
        )
    ...
    def run_check_view(self, request, object_id, topic):
        ...
        if handler is None:
            ...
            return HttpResponseRedirect(reverse("admin:ordering_session_change", args=[object_id]))
...
class OrderAdmin(ModelAdmin):
    ...
    def advance_status_row(self, request, object_id):
        ...
        if order is None:
            ...
            return HttpResponseRedirect(reverse("admin:ordering_order_changelist"))
...
class DirectiveAdmin(ModelAdmin):
    ...
    def execute_now_detail_action(self, request, object_id):
        ...
        if directive is None:
            ...
            return HttpResponseRedirect(reverse("admin:ordering_directive_changelist"))
        ...
        return HttpResponseRedirect(reverse("admin:ordering_directive_change", args=[object_id]))
