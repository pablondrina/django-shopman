...
def default_expiry():
    ...
    return timezone.now() + timedelta(minutes=gating_settings.BRIDGE_TOKEN_TTL_MINUTES)
...
class BridgeToken(models.Model):
    ...
    Flow:
    1. Manychat/backend calls POST /gating/bridge-tokens
    2. Receives URL with token
    3. Sends to customer
    4. Customer clicks -> GET /gating/bridge/exchange?t=...
    5. Gating validates, creates session, redirects
    ...
    user = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="gating_bridge_tokens",
        verbose_name=_("usuário"),
    )

    class Meta:
        db_table = "gating_bridge_token"
