...
from ..conf import gating_settings, get_customer_resolver, get_gating_settings
...
logger = logging.getLogger("shopman.gating.verification")
...
class VerificationService:
    ...
    def request_code(
        ...
        try:
            Gates.rate_limit(
                key=target_value,
                max_requests=gating_settings.CODE_RATE_LIMIT_MAX,
                window_minutes=gating_settings.CODE_RATE_LIMIT_WINDOW_MINUTES,
            )
        except GateError:
        ...
        try:
            Gates.code_cooldown(
                ...
                cooldown_seconds=gating_settings.MAGIC_CODE_COOLDOWN_SECONDS,
            )
        except GateError:
            return CodeRequestResult(
                success=False,
    ...
    def verify_for_login(
        ...
        if not customer:
            ...
            if not get_gating_settings().AUTO_CREATE_CUSTOMER:
                return VerifyResult(
                    success=False,
    ...
    def _get_default_sender(cls):
        ...
        sender_class = import_string(gating_settings.MESSAGE_SENDER_CLASS)
        return sender_class()
