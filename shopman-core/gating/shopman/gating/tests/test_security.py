...
from shopman.gating.conf import get_gating_settings
from shopman.gating.exceptions import GateError
from shopman.gating.gates import Gates
from shopman.gating.models import BridgeToken, MagicCode
from shopman.gating.models.magic_code import generate_code, generate_raw_code, verify_code
...
from shopman.gating.utils import safe_redirect_url
from shopman.gating.views.bridge import BridgeTokenCreateView, BridgeTokenExchangeView
...
class TestOTPGeneration:
    ...
    def test_otp_uses_secrets_module(self):
        ...
        import shopman.gating.models.magic_code as mc_module
...
class TestSafeRedirectUrl:
    ...
    @override_settings(GATING={"ALLOWED_REDIRECT_HOSTS": {"trusted.com"}})
...
class TestBridgeTokenCreateAuth:
    ...
    def _post_create(self, data, headers=None):
        ...
        Mocks _build_url to avoid reverse() failing when gating URLs
    ...
    @override_settings(GATING={"BRIDGE_TOKEN_API_KEY": TEST_API_KEY})
...
class TestBridgeTokenLifecycle:
    ...
    def test_invalid_token_does_not_redirect(self):
        ...
        factory = RequestFactory()
        request = factory.get("/gating/bridge/", {"t": "invalid-token-123"})
        from django.contrib.sessions.backends.db import SessionStore
        ...
        with patch("shopman.gating.views.bridge.render", return_value=None) as mock_render:
            view(request)
...
class TestCustomerAutoCreation:
    ...
    @override_settings(GATING={"AUTO_CREATE_CUSTOMER": False})
...
class TestPIILogging:
    ...
    def test_session_values_not_in_info_logs(self, customer, bridge_token):
        ...
        with patch("shopman.gating.services.auth_bridge.logger") as mock_logger:
