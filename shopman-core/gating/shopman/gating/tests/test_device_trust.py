...
from shopman.gating.models import TrustedDevice
...
class TestDeviceTrustService:
    ...
    def test_check_device_trust_valid(self):
        ...
        cid = uuid.uuid4()
        device, raw_token = TrustedDevice.create_for_customer(customer_id=cid)
        request = self._make_request(cookies={"gating_dt": raw_token})
    ...
    @override_settings(GATING={"DEVICE_TRUST_ENABLED": False})
    ...
    def test_trust_device_sets_cookie(self):
        ...
        assert "gating_dt" in response.cookies
        cookie = response.cookies["gating_dt"]
    ...
    def test_trust_device_emits_signal(self):
        ...
        from shopman.gating.signals import device_trusted
    ...
    def test_revoke_device_with_valid_cookie(self):
        ...
        assert "gating_dt" in response.cookies
        assert response.cookies["gating_dt"]["max-age"] == 0
