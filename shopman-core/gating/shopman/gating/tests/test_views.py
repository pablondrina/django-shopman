"""
Integration tests for Gating views (Phase 4.2).

Tests the full request/verify flow via RequestFactory, including
form and JSON content negotiation.

Note: Since gating URLs may not be registered in the test project,
...
from shopman.gating.models import MagicCode
from shopman.gating.models.magic_code import generate_raw_code
from shopman.gating.services.auth_bridge import AuthBridgeService
from shopman.gating.views.bridge import BridgeTokenCreateView
from shopman.gating.views.health import HealthCheckView
from shopman.gating.views.magic_code import MagicCodeRequestView, MagicCodeVerifyView

# Patch _build_url globally for all verify-success tests since
# gating URLs are not registered in the test project.
_PATCH_BUILD_URL = patch.object(
    AuthBridgeService, "_build_url", return_value="https://test.local/gating/bridge/?t=mock"
)
...
class TestMagicCodeRequestViewForm:
    ...
    @patch("shopman.gating.views.magic_code.redirect", side_effect=_fake_redirect)
    @patch("shopman.gating.services.verification.VerificationService._get_default_sender")
    def test_post_valid_phone_redirects(self, mock_sender, mock_redirect):
        ...
        assert request.session.get("gating_phone") == "+5541999999999"
...
class TestMagicCodeVerifyViewForm:
    ...
    def test_get_with_session_phone_renders(self):
        ...
        session["gating_phone"] = "+5541999999999"
        session.save()
...
TEST_API_KEY = "test-gating-api-key-2026"
...
class TestBridgeTokenCreateViewEdges:
    ...
    def _post_create(self, data, headers=None):
        ...
        request = factory.post("/gating/bridge/create/", body, **kwargs)
        view = BridgeTokenCreateView.as_view()
        with patch.object(
            AuthBridgeService,
            "_build_url",
            return_value="https://test.local/gating/bridge/?t=mock",
        ):
            return view(request)

    @override_settings(GATING={"BRIDGE_TOKEN_API_KEY": ""})
    def test_create_invalid_json(self):
        """Invalid JSON should return 400."""
        factory = RequestFactory()
        request = factory.post(
            "/gating/bridge/create/",
            "not-json",
    ...
    def test_create_customer_inactive(self, customer):
        ...
        with patch("shopman.gating.views.bridge.get_customer_resolver") as mock_resolver:
...
class TestHealthCheckView:
    ...
    def test_health_returns_200(self):
        factory = RequestFactory()
        request = factory.get("/gating/health/")
