...
logger = logging.getLogger("shopman.gating.views.magic_code")
...
class MagicCodeRequestView(View):
    ...
    def post(self, request):
        ...
        request.session["gating_phone"] = phone
        ...
        if raw_next:
            ...
            if validated != get_gating_settings().LOGIN_REDIRECT_URL:
                request.session["doorman_next"] = validated

        return redirect("gating:code-verify")
...
class MagicCodeVerifyView(View):
    ...
    def get(self, request):
        phone = request.session.get("gating_phone")
        if not phone:
            return redirect("gating:code-request")
    ...
    def post(self, request):
        ...
        if is_json:
            ...
            phone = request.POST.get("phone") or request.session.get("gating_phone", "")
        ...
        # Clear session data
        request.session.pop("gating_phone", None)
