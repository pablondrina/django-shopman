...
logger = logging.getLogger("shopman.gating.senders")
...
class ConsoleSender:
    ...
    def send_code(self, target: str, code: str, method: str) -> bool:
        print(f"\n{'='*50}")
        print(f"GATING - Verification Code")
...
class WhatsAppCloudAPISender:
    ...
    def __init__(self):
        from .conf import gating_settings

        self.access_token = gating_settings.WHATSAPP_ACCESS_TOKEN
        self.phone_id = gating_settings.WHATSAPP_PHONE_ID
        self.template_name = gating_settings.WHATSAPP_CODE_TEMPLATE
...
class EmailSender:
    ...
    def send_code(self, target: str, code: str, method: str) -> bool:
        ...
        ttl = gating_settings.MAGIC_CODE_TTL_MINUTES
        context = {"code": code, "ttl_minutes": ttl}

        try:
            subject = _("Your verification code")
            text_body = render_to_string("gating/email_code.txt", context)
            html_body = render_to_string("gating/email_code.html", context)
