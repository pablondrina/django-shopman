...
from shopman.auth.senders import (
    ConsoleSender,
    EmailSender,
    LogSender,
    SMSSender,
...
class TestLogSender:
    ...
    def test_send_code_logs_message(self, caplog):
        sender = LogSender()
        with caplog.at_level(logging.INFO, logger="shopman.auth.senders"):
...
class TestSMSSender:
    ...
    def test_send_code_does_not_log_raw_code(self, caplog):
        ...
        sender = SMSSender()
        with caplog.at_level(logging.WARNING, logger="shopman.auth.senders"):
