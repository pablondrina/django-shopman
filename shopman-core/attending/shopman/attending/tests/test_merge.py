...
from shopman.attending.contrib.merge.models import MergeAudit, MergeStatus
from shopman.attending.contrib.merge.service import MergeResult, MergeService
from shopman.attending.exceptions import AttendingError
from shopman.attending.gates import GateError
from shopman.attending.models import (
    ContactPoint,
    Customer,
    CustomerAddress,
    CustomerGroup,
...
from shopman.attending.contrib.identifiers.models import CustomerIdentifier, IdentifierType
from shopman.attending.contrib.preferences.models import CustomerPreference, PreferenceType
from shopman.attending.contrib.insights.models import CustomerInsight
from shopman.attending.contrib.consent.models import (
    CommunicationConsent,
    ConsentChannel,
    ConsentStatus,
    LegalBasis,
)
from shopman.attending.contrib.loyalty.models import (
    LoyaltyAccount,
    LoyaltyTransaction,
    LoyaltyTier,
    TransactionType,
)
from shopman.attending.contrib.timeline.models import EventType, TimelineEvent
...
class TestMergeUndo:
    ...
    def test_undo_rejects_already_reverted(self, source, target, evidence):
        ...
        with pytest.raises(AttendingError, match="already reverted"):
            MergeService.undo(result.audit_id, actor="test")
    ...
    def test_undo_rejects_expired_window(self, source, target, evidence):
        ...
        with pytest.raises(AttendingError, match="window expired"):
            MergeService.undo(result.audit_id, actor="test")

    def test_undo_rejects_invalid_audit_id(self):
        with pytest.raises(AttendingError, match="not found"):
