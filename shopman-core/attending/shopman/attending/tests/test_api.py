"""Tests for Attending REST API (SPEC-002)."""
...
from shopman.attending.models import Customer, CustomerAddress, CustomerGroup
...
@pytest.fixture
def group_vip(db):
    return CustomerGroup.objects.create(ref="vip", name="VIP", listing_code="vip", priority=10)
...
def customer_insight(db, customer):
    from shopman.attending.contrib.insights.models import CustomerInsight
...
class TestCustomerList:
    """GET /api/attending/customers/"""

    def test_list_active_customers(self, api_client, customer, customer_vip):
        resp = api_client.get("/api/attending/customers/")
    ...
    def test_filter_by_group(self, api_client, customer, customer_vip):
        resp = api_client.get("/api/attending/customers/", {"group": "vip"})
        codes = {c["ref"] for c in resp.data["results"]}
        assert "CUST-VIP" in codes
        assert "CUST-001" not in codes

    def test_search_by_name(self, api_client, customer, customer_vip):
        resp = api_client.get("/api/attending/customers/", {"search": "John"})
        codes = {c["ref"] for c in resp.data["results"]}
        assert "CUST-001" in codes
        assert "CUST-VIP" not in codes

    def test_search_by_phone(self, api_client, customer):
        resp = api_client.get("/api/attending/customers/", {"search": "999999999"})
        codes = {c["ref"] for c in resp.data["results"]}
        assert "CUST-001" in codes

    def test_search_by_email(self, api_client, customer):
        resp = api_client.get("/api/attending/customers/", {"search": "john@example"})
        codes = {c["ref"] for c in resp.data["results"]}
        assert "CUST-001" in codes
    ...
    def test_serializer_fields(self, api_client, customer):
        ...
        expected = {
            ...
            "phone", "phone_display", "email", "group_name", "listing_code", "is_active",
        }
        assert set(cust.keys()) == expected


class TestCustomerDetail:
    """GET /api/attending/customers/{ref}/"""

    def test_retrieve_by_code(self, api_client, customer):
        resp = api_client.get(f"/api/attending/customers/{customer.ref}/")
        assert resp.status_code == 200
    ...
    def test_404_for_nonexistent(self, api_client):
        resp = api_client.get("/api/attending/customers/NOPE/")
        assert resp.status_code == 404

    def test_404_for_inactive(self, api_client, customer_inactive):
        resp = api_client.get(f"/api/attending/customers/{customer_inactive.ref}/")
        assert resp.status_code == 404


class TestCustomerCreate:
    """POST /api/attending/customers/"""

    def test_create_customer(self, api_client, group_regular):
        resp = api_client.post(
            "/api/attending/customers/",
    ...
    def test_create_generates_contact_point(self, api_client, group_regular):
        ...
        ref = resp.data["ref"]
        detail = api_client.get(f"/api/attending/customers/{ref}/")
...
class TestCustomerUpdate:
    """PATCH /api/attending/customers/{ref}/"""

    def test_update_first_name(self, api_client, customer):
        resp = api_client.patch(
            f"/api/attending/customers/{customer.ref}/",
            {"first_name": "Jonathan"},
            format="json",
        )
        assert resp.status_code == 200
    ...
    def test_delete_not_allowed(self, api_client, customer):
        resp = api_client.delete(f"/api/attending/customers/{customer.ref}/")
...
class TestContacts:
    """GET /api/attending/customers/{ref}/contacts/"""

    def test_list_contacts(self, api_client, customer):
        resp = api_client.get(f"/api/attending/customers/{customer.ref}/contacts/")
        assert resp.status_code == 200
...
class TestAddresses:
    """GET/POST /api/attending/customers/{ref}/addresses/"""

    def test_list_addresses(self, api_client, customer, customer_address):
        resp = api_client.get(f"/api/attending/customers/{customer.ref}/addresses/")
        assert resp.status_code == 200
        assert len(resp.data) == 1

    def test_create_address(self, api_client, customer):
        resp = api_client.post(
            f"/api/attending/customers/{customer.ref}/addresses/",
            {
                "label": "work",
...
class TestLookup:
    """GET /api/attending/lookup/"""

    def test_lookup_by_phone(self, api_client, customer):
        resp = api_client.get("/api/attending/lookup/", {"phone": "+5543999999999"})
        assert resp.status_code == 200
        assert resp.data["ref"] == "CUST-001"

    def test_lookup_by_email(self, api_client, customer):
        resp = api_client.get("/api/attending/lookup/", {"email": "john@example.com"})
        assert resp.status_code == 200
        assert resp.data["ref"] == "CUST-001"

    def test_lookup_not_found(self, api_client, customer):
        resp = api_client.get("/api/attending/lookup/", {"phone": "+5543000000000"})
        assert resp.status_code == 404

    def test_lookup_requires_param(self, api_client):
        resp = api_client.get("/api/attending/lookup/")
        assert resp.status_code == 400
...
class TestInsights:
    """GET /api/attending/customers/{ref}/insights/"""

    def test_get_insights(self, api_client, customer, customer_insight):
        resp = api_client.get(f"/api/attending/customers/{customer.ref}/insights/")
        assert resp.status_code == 200
    ...
    def test_insights_not_found(self, api_client, customer_vip):
        resp = api_client.get(f"/api/attending/customers/{customer_vip.ref}/insights/")
        assert resp.status_code == 404


class TestInsightsSummary:
    """GET /api/attending/insights/summary/"""

    def test_summary(self, api_client, customer, customer_insight):
        resp = api_client.get("/api/attending/insights/summary/")
        assert resp.status_code == 200
...
class TestPreferences:
    """GET/PATCH /api/attending/customers/{ref}/preferences/"""

    def test_get_preferences(self, api_client, customer, customer_preference):
        resp = api_client.get(f"/api/attending/customers/{customer.ref}/preferences/")
        assert resp.status_code == 200
    ...
    def test_update_preferences(self, api_client, customer):
        resp = api_client.patch(
            f"/api/attending/customers/{customer.ref}/preferences/",
...
class TestAuthentication:
    ...
    def test_customers_requires_auth(self, anon_client, customer):
        resp = anon_client.get("/api/attending/customers/")
        assert resp.status_code == 403

    def test_lookup_requires_auth(self, anon_client, customer):
        resp = anon_client.get("/api/attending/lookup/", {"phone": "+5543999999999"})
        assert resp.status_code == 403

    def test_insights_summary_requires_auth(self, anon_client):
        resp = anon_client.get("/api/attending/insights/summary/")
        assert resp.status_code == 403
