...
class ContactPoint(models.Model):
    ...
    - OTP_WHATSAPP: Code sent via WhatsApp (via Doorman)
    - OTP_SMS: Code sent via SMS (via Doorman)
    - EMAIL_LINK: Verification link by email (via Doorman)
    ...
    class Meta:
        db_table = "attending_contact_point"
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name="attending_unique_contact_value",
            ),
            models.UniqueConstraint(
                fields=["customer", "type"],
                condition=models.Q(is_primary=True),
                name="attending_unique_primary_per_type",
            ),
        ]
        indexes = [
    ...
    def normalize_value(value: str, contact_type: str | None = None) -> str:
        ...
        """
        from shopman.attending.utils import normalize_phone
