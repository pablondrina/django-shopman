...
from shopman.attending.contrib.loyalty.conf import get_tier_thresholds
...
class LoyaltyService:
    ...
    def earn_points(
        ...
        Raises:
            AttendingError: If not enrolled or points <= 0
        """
        if points <= 0:
            raise AttendingError("LOYALTY_INVALID_POINTS", message="Points must be positive")
    ...
    def redeem_points(
        ...
        Raises:
            AttendingError: If not enrolled, insufficient balance, or points <= 0
        """
        if points <= 0:
            raise AttendingError("LOYALTY_INVALID_POINTS", message="Points must be positive")
    ...
    def add_stamp(
        ...
        Raises:
            AttendingError: If not enrolled
        """
        with transaction.atomic():
            account = cls._get_active_account_for_update(customer_ref)
    ...
    def _get_active_account(cls, customer_ref: str) -> LoyaltyAccount:
        ...
        try:
            ...
            raise AttendingError("LOYALTY_NOT_ENROLLED", customer_ref=customer_ref)
