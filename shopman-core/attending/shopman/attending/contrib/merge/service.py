...
class MergeService:
    ...
    def merge(
        ...
        Raises:
            AttendingError: If gate validation fails or customers are invalid.
        """
        # --- Validate via G6 ---
        Gates.merge_safety(
    ...
    def undo(cls, audit_id: str, actor: str = "") -> None:
        ...
        Raises:
            AttendingError: If audit not found, already reverted, or window expired.
        """
        ...
        try:
            ...
            raise AttendingError("UNDO_FAILED", message="Merge audit record not found.")

        if audit.status != MergeStatus.COMPLETED:
            raise AttendingError("UNDO_FAILED", message="Merge already reverted.")
        ...
        with transaction.atomic():
            ...
            if consent_pks:
                try:
                    from shopman.attending.contrib.consent.models import CommunicationConsent
    ...
    def _create_audit(
        ...
        from shopman.attending.contrib.merge.models import MergeAudit
