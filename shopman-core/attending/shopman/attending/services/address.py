...
from shopman.attending.services.customer import get
...
def add_address(
    ...
    cust = get(customer_ref)
    if not cust:
        raise AttendingError("CUSTOMER_NOT_FOUND", customer_ref=customer_ref)
...
def set_default_address(customer_ref: str, address_id: int) -> CustomerAddress:
    ...
    with transaction.atomic():
        try:
            ...
            raise AttendingError("ADDRESS_NOT_FOUND", address_id=address_id)
